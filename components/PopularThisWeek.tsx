...
const PopularEpisodeCard: React.FC<{episode: Episode}> = ({episode}) => (
    <div className="group flex gap-4 p-4 bg-white rounded-lg border border-gray-100 hover:shadow-lg transition-shadow duration-200">
        ...
        <div className="min-w-0">
            <h4 className="font-semibold text-richblack line-clamp-2 group-hover:text-primary-600">{episode.episodeTitle}</h4>
                ...
                <ul className="mt-4 space-y-3">
                    {trendingTopics.map((topic, index) => (
                        <li key={topic.id} className="flex items-center justify-between text-gray-700">
                           <a href="#" className="hover:text-primary-600 transition-colors"> <span className="mr-2 text-gray-400">{index + 1}.</span>{topic.name}</a>
